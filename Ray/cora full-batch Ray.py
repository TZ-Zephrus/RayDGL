...
from dgl.data import CoraGraphDataset, CoraFullDataset, PubmedGraphDataset, FraudDataset
...
# 数据准备
dataset = PubmedGraphDataset(raw_dir="/root/wtz/RayDGL/dataset/pubmed") # Cora citation network dataset
graph = dataset[0]
...
for i in range(3):
    ...
    ) = dgl.distributed.load_partition('/root/wtz/RayDGL/dataset/pubmed 3 partition/pubmed.json', i)
...
workers2 = workers[0:-1] # 只有前两个worker
...
for i in range(epoch):
    ...
    # gradients = [worker.compute_gradients.remote(current_weights) for worker in workers2]   # 尝试只使用两个worker进行训练
